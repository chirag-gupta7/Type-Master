...
export const getWeakKeyAnalysis = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // OPTIMIZATION: Parallelize independent queries using Promise.all to reduce latency.
    // Before: 3 sequential database queries (userWeakKeys, raw finger errors query, typingMistake history).
    //   Total latency = Latency(Q1) + Latency(Q2) + Latency(Q3)
    // After: 3 parallel database queries executed concurrently.
    //   Total latency = Max(Latency(Q1), Latency(Q2), Latency(Q3))
    // This reduces the endpoint's response time significantly and prevents thread blocking.
