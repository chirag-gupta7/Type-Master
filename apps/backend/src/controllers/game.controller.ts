...
const saveGameScoreSchema = z.object({
  ...
  score: z.number().min(0, 'Invalid score value').max(1000000, 'Score is unrealistically high'),
  wpm: z.number().min(0, 'WPM must be non-negative').max(300, 'WPM is unrealistically high').nullable().optional(),
  accuracy: z.number().min(0, 'Accuracy must be at least 0').max(100, 'Accuracy cannot exceed 100').nullable().optional(),
  duration: z.number().min(0, 'Duration must be non-negative').max(86400, 'Duration is unrealistically high').nullable().optional(),
  ...
  try {
    ...
    const gameScore = await prisma.gameScore.create({
      data: {
        ...
        duration: payload.duration !== undefined && payload.duration !== null ? Math.trunc(payload.duration) : null,
        metadata: serializeMetadata(payload.metadata as Record<string, unknown> | undefined),
      },
      include: {
        user: withUser,
      },
    ...
    if (error instanceof z.ZodError) {
      const issueMessage = error.errors[0]?.message;
      if (issueMessage === 'Invalid game type' || issueMessage === 'Invalid score value') {
