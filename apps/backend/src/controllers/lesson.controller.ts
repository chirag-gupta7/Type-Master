...
export const getLearningStats = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const [totalLessons, completedProgress, userAggregates] = await Promise.all([
      prisma.lesson.count(),
    ...
    const totalStars = userAggregates._sum.stars || 0;
    const avgWpm = userAggregates._avg.bestWpm || 0;
    const avgAccuracy = userAggregates._avg.bestAccuracy || 0;
