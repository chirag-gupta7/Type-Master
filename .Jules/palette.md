...
**Action:** Always wrap icon-only buttons in a Tooltip and provide a clear `aria-label`. Use `asChild` on the TooltipTrigger to preserve button semantics. Additionally, ensure `TooltipProvider` is globally available to enable tooltip functionality throughout the app.
